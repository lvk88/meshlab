...
class DecorateShadowPlugin : public QObject, public DecoratePlugin
{
	...
	enum { DP_SHOW_SHADOW, DP_SHOW_SSAO };
	...
	{
		return QStringList() << "Shadow mapping"
...
public:
	...
	{
		typeList << DP_SHOW_SHADOW << DP_SHOW_SSAO;
	...
	bool            startDecorate(
				   const QAction* /*mode*/,
				   MeshDocument& /*m*/,
				   const RichParameterList* /*parent*/ par,
				   GLArea* /*parent*/);
	void decorateMesh(
		const QAction*,
		MeshModel&,
...
private:
	...
	inline const QString DecorateShadowIntensity()
	{
		return "MeshLab::Decoration::ShadowIntensityVal";
	}
};
