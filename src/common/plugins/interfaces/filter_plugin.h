...
public:
	/**
	 * @brief The FilterClass enum represents the set of keywords that must be used to categorize a
	 * filter. Each filter can belong to one or more filtering class, or-ed together.
	 */
	enum FilterClass {
		Generic = 0x00000,
		/*!< Should be avoided if possible. */ //
		Selection = 0x00001, /*!<  select or de-select something, basic operation on selections
								(like deleting)*/
		Cleaning =
			0x00002, /*!<  Filters that can be used to clean meshes (duplicated vertices etc)*/
		Remeshing =
			0x00004, /*!<  Simplification, Refinement, Reconstruction and mesh optimization*/
		FaceColoring   = 0x00008,
		VertexColoring = 0x00010,
		MeshColoring   = 0x00020,
		MeshCreation   = 0x00040,
		Smoothing =
			0x00080, /*!<  Stuff that does not change the topology, but just the vertex positions*/
		Quality     = 0x00100,
		Layer       = 0x00200, /*!<  Layers, attributes */
		RasterLayer = 0x00400, /*!<  Raster Layers, attributes */
		Normal = 0x00800, /*!<  Normal, Curvature, orientation (rotations and transformations fall
							 here)*/
		Sampling  = 0x01000,
		Texture   = 0x02000,
	 ...
	 * For example filters that perform an action only on the selection will be placed in the
	 * Selection Class
	 */
	 ...
	 * The framework will ensure that the mesh has the requirements satisfied before invoking the
	...
	/**
	 * @brief The FilterPrecondition mask is used to explicitate what kind of data a filter really
	 * needs to be applied. For example algorithms that compute per face quality have as
	 * precondition the existence of faces (but quality per face is not a precondition, because
	 * quality per face is created by these algorithms) on the other hand an algorithm that deletes
	 * faces according to the stored quality has both FaceQuality and Face as precondition. These
	 * conditions do NOT include computed properties like borderFlags, manifoldness or
	 * watertightness. They are also used to grayout menus un-appliable entries.
	 */
	virtual int getPreConditions(const QAction*) const { return MeshModel::MM_NONE; }

	/**
	 * @brief Function used by the framework to get info about the mesh properties changed by the
	 * filter. It is widely used by the meshlab's preview system. TO BE REPLACED WITH = 0
	 */
	 ...
	 * This function is called by the framework after getting values for the parameters specified in
	 * the \ref initParameterList NO GUI interaction should be done here. No dialog asking, no
	 * messagebox errors. Think that his function will also be called by the commandline framework.
	 ...
	 * When implementing your applyFilter, you should use the cb function to report to the framework
	 * the current state of the processing. During your (long) processing you should call from time
	 * to time cb(perc,descriptiveString), where perc is an int (0..100) saying what you are doing
	 * and at what point of the computation you currently are. The function returns a map of
	 * [string-value] pairs, that will be output of command-line calls of the filter. If your filter
	 * does not return nothing, just return an empty map.
	 * @sa errorMsg
	 * @sa initParameterSet
	 */
	 ...
	 * For instance a colorize by quality filter cannot be applied to a mesh without
	 * per-vertex-quality. On failure (returning false) the function fills the MissingItems list
	 * with strings describing the missing items.
	 */
	bool
	isFilterApplicable(const QAction* act, const MeshModel& m, QStringList& MissingItems) const;
	 ...
	 * - FIXED: the number (and the names) of the meshes involved in the filter computation is
	 * determined by the parameters selected in the filter's parameters form
	 * - VARIABLE: the filter works on a not predetermined number of meshes. The meshes involved are
	 * typically selected by the user checking on the correspondent layer on the layer dialog
	 */
	...
	virtual QString pythonFilterName(const QAction* a) const
	{
		return this->pythonFilterName(ID(a));
	}
	...
	/**
	 * Generate the mask of attributes would be created IF the MeshFilterInterface filt would has
	 * been called on MeshModel mm BE CAREFUL! this function does NOT change in anyway the state of
	 * the MeshModel!!!!
	 */
...
protected:
	...
	// Each filtering procedure corresponds to a single QAction with a corresponding FilterIDType
	// id.
	//
